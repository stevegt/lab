...
func main() {
	...
	tests := []struct {
		...
		{[]float64{587000, 16.5, 6.2}, 11.2},
		{[]float64{643000, 20.5, 6.4}, 13.4},
		{[]float64{635000, 26.3, 9.3}, 40.7},
		{[]float64{80000, 16.3, 4.3}, math.NaN()},
	}
	for _, test := range tests {
		predicted, err := r.Predict(test.input)
		if err != nil {
			fmt.Printf("Error: %v\n", err)
		} else {
			fmt.Printf("population: %v, low income: %v, unemployed: %v, predicted: %v, expected: %v\n", test.input[0], test.input[1], test.input[2], predicted, test.expected)
		}
	}
